...
test("round-trips PSpice ABM CHEBYSHEV VCVS exactly", () => {
  const source = readFileSync(
    "tests/assets/pspice-specific/abm-chebyshev-vcvs.cir",
    "utf8",
  )
  ...
  expect(() => netlist.getString()).toThrowErrorMatchingInlineSnapshot(
    `"PSpice-only construct CHEBYSHEV behavioral voltage source cannot be serialized to target dialect "generic". Serialize with { dialect: "pspice" } or translate/remove the construct before exporting."`,
  )
})
