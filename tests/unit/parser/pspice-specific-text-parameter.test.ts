...
test("round-trips PSpice TEXT parameters exactly", () => {
  const source = readFileSync(
    "tests/assets/pspice-specific/text-parameter.cir",
    "utf8",
  )
  ...
  expect(() => netlist.getString()).toThrowErrorMatchingInlineSnapshot(
    `"PSpice-only construct .TEXT cannot be serialized to target dialect "generic". Serialize with { dialect: "pspice" } or translate/remove the construct before exporting."`,
  )
})
