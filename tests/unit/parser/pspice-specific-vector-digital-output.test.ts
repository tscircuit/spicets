...
test("round-trips PSpice VECTOR output exactly", () => {
  const source = readFileSync(
    "tests/assets/pspice-specific/vector-digital-output.cir",
    "utf8",
  )
  ...
  expect(() => netlist.getString()).toThrowErrorMatchingInlineSnapshot(
    `"PSpice-only construct .VECTOR cannot be serialized to target dialect "generic". Serialize with { dialect: "pspice" } or translate/remove the construct before exporting."`,
  )
})
