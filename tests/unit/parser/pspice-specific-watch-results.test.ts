...
test("round-trips PSpice WATCH results exactly", () => {
  const source = readFileSync(
    "tests/assets/pspice-specific/watch-results.cir",
    "utf8",
  )
  ...
  expect(() => netlist.getString()).toThrowErrorMatchingInlineSnapshot(
    `"PSpice-only construct .WATCH cannot be serialized to target dialect "generic". Serialize with { dialect: "pspice" } or translate/remove the construct before exporting."`,
  )
})
