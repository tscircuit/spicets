...
test("round-trips PSpice ABM VALUE VCCS exactly", () => {
  ...
  expect(() => netlist.getString()).toThrowErrorMatchingInlineSnapshot(
    `"PSpice-only construct VALUE behavioral current source cannot be serialized to target dialect "generic". Serialize with { dialect: "pspice" } or translate/remove the construct before exporting."`,
  )
})
